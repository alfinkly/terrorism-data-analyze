...
def rank_countries_by_metric(df, metric, ascending=False):
    ...
    if metric == 'Attacks':
        ...
        ranking = df.groupby('Country')['Casualties'].sum().sort_values(ascending=ascending).reset_index()
    else:
...
def analyze_kazakhstan_data(df):
    ...
    sns.countplot(y='AttackType', data=kaz_df, order=kaz_df['AttackType'].value_counts().index)
    ...
    plt.figure(figsize=(10, 6))
    sns.countplot(y='Target_type', data=kaz_df, order=kaz_df['Target_type'].value_counts().index)
...
if __name__ == "__main__":
    ...
    if gtd_df is not None:
        gtd_df.rename(columns={'iyear':'Year','imonth':'Month','iday':'Day','country_txt':'Country','provstate':'state',
        ...
        kaz_casualties_rank = casualties_ranking[casualties_ranking['Country'] == 'Kazakhstan']
